...
Uses a neural network to score actions and selects via softmax sampling. The network architecture and training are
defined separately.
...
class LearnedSelector(ActionSelector):
    ...
    The network takes state information and candidate actions as input, outputs logits for each action, applies softmax,
