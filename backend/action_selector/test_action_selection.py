...
def test_epsilon_greedy_propensity():
    ...
    epsilon = 0.1
    manager = ActionSelectorManager(mode="epsilon_greedy", epsilon=epsilon, learned_model_path=None, device="cpu")
    ...
    for i in range(num_trials):
        ...
        assert abs(propensity - expected_prop) < 1e-6, f"Propensity mismatch: {propensity} != {expected_prop}"

        # Random propensity should always be 1/n
        assert abs(random_prop - 1.0 / n) < 1e-6, f"Random propensity should be {1.0/n}, got {random_prop}"
    ...
    assert abs(random_fraction - epsilon) < 0.05, f"Random fraction {random_fraction} too far from epsilon {epsilon}"
...
def test_propensity_coverage():
    ...
    for i, count in enumerate(selection_counts):
        ...
        assert (
            abs(count - expected_count) < expected_count * 0.3
