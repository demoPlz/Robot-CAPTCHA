...
def euler_pose(x: float, y: float, z: float, roll: float, pitch: float, yaw: float) -> list[list[float]]:
    """Build a 4x4 **world** matrix (row-major list-of-lists) from T = Trans(x,y,z) · Rz(yaw) · Ry(pitch) · Rx(roll)"""
...
def submit_goal():
    """{ "joint_positions": { "joint_0": 0.12, … }, "gripper": -1,        # -1 close, +1 open, 0 unchanged "ee_pose": {
...
def robot_loop(record: bool = False):
    ...
    driver.configure(trossen_arm.Model.wxai_v0, trossen_arm.StandardEndEffector.wxai_v0_leader, server_ip, True)

    convert_rad = np.pi / 180.0

    print("Moving to home positions...")
    driver.set_all_modes(trossen_arm.Mode.position)
    driver.set_all_positions(np.array([0.0, 60.0, 75.0, -60.0, 0.0, 0.0, 2.0]) * convert_rad, 2.0, True)
    ...
    try:
        ...
        while True:
            ...
            if submission:
                goal_positions = np.array([submission["joint_positions"][n] for n in joint_names], dtype=float)

                driver.set_all_positions(goal_positions, goal_time=2.0, blocking=False)
    ...
    print("Moving to home positions...")
    driver.set_all_modes(trossen_arm.Mode.position)
    driver.set_all_positions(np.array([0.0, np.pi / 2, np.pi / 2, 0.0, 0.0, 0.0, 0.0]), 2.0, True)
...
if __name__ == "__main__":
    ...
    server_thread = Thread(
        target=lambda: app.run(host="0.0.0.0", port=9000, debug=False, use_reloader=False), daemon=True
    )
    server_thread.start()
