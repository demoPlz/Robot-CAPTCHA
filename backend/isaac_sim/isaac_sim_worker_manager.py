#!/usr/bin/env python3
"""Backend integration for persistent Isaac Sim worker Manages long-running Isaac Sim subprocess for state cycling."""
...
class PersistentWorkerManager:
    ...
    def start_worker(self, initial_config: Dict[str, Any], max_users: int = 10):
        ...
        if "PATH" in clean_env:
            ...
            clean_path_parts = [
                p for p in path_parts if not any(x in p.lower() for x in ["conda", "miniconda", "anaconda"])
            ]
    ...
    def _cleanup_expired_animations(self):
        """Background thread to cleanup animations that exceed timeout (10 seconds) or have no activity."""
    ...
    def capture_initial_state(self, config: Dict[str, Any]) -> Dict[str, str]:
        ...
        command = {"action": "initialize_and_capture", "config": config, "output_dir": output_dir}
        ...
        if result.get("status") == "success":
            ...
            try:
                anim_command = {"action": "initialize_animation", "max_users": self.max_animation_users}
    ...
    def stop_user_animation(self, user_id: int) -> Dict[str, Any]:
        ...
        command = {"action": "stop_user_animation", "user_id": user_id}
        ...
        if self.animation_initialized and result.get("status") == "success":
            try:
                ...
                if sync_result.get("status") != "success":
                    print(
                        f"Warning: Failed to sync animation environments: {sync_result.get('message', 'Unknown error')}"
                    )
            except Exception as e:
    ...
    def start_user_animation_managed(
        self,
        session_id: str,
        goal_pose: Dict = None,
        goal_joints: list = None,
    ...
    def capture_user_frame(self, user_id: int) -> Dict[str, Any]:
        ...
        # Update last activity timestamp to detect stale sessions
        if user_id in self.animation_users:
            self.animation_users[user_id]["last_activity"] = time.time()
...
class IsaacSimBackend:
    ...
    def start_user_animation(
        self, user_id: int, goal_pose: Dict = None, goal_joints: list = None, duration: float = 3.0
    ) -> Dict[str, Any]:
