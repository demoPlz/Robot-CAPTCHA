#!/usr/bin/env python3
"""Persistent Isaac Sim worker that stays running and processes commands via file communication This script runs in
Isaac Sim's Python environment and communicates with backend via JSON files."""
    ...
    def _process_command(self, command: dict) -> dict:
        ...
        try:
            if action == "initialize_and_capture":
                ...
                return {"status": "success", "action": action, "message": "Animation loop completed"}
                ...
                if not self.isaac_worker.animation_mode:
                    return {"status": "error", "action": action, "message": "Animation mode not initialized"}

                self.isaac_worker.sync_animation_environments(config)

                return {"status": "success", "action": action, "message": "Animation environments synchronized"}
            ...
            return {"status": "error", "action": action, "message": str(e), "traceback": traceback.format_exc()}
