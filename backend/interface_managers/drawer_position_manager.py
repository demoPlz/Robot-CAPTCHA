...
Handles drawer position estimation using ArUco markers from observation images. Works with saved observation data (like
pose estimation) rather than direct camera access.
...
class DrawerPositionManager:
    ...
    def _estimate_marker_pose(self, corners):
        ...
        success, rvec, tvec = cv2.solvePnP(obj_points, img_points, self.K, self.D, flags=cv2.SOLVEPNP_IPPE_SQUARE)
    ...
    def get_joint_position_from_obs(self, obs_dict: dict) -> dict | None:
        ...
        print(
            f"🗄️  Drawer joint position: {self.drawer_joint_name} = {joint_position:.4f} m (displacement: {displacement*100:.2f} cm)"
        )
