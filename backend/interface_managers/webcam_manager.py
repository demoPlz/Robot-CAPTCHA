...
Handles webcam capture, background encoding, and frame streaming for the crowd interface. Manages V4L2 camera devices
with RealSense filtering and undistortion.
...
_REALSENSE_BLOCKLIST = ("realsense", "real sense", "d4", "depth", "infrared", "stereo module", "motion module")
...
class WebcamManager:
    ...
    def __init__(
        self, cam_ids: dict[str, int], undistort_maps: dict[str, tuple] | None = None, jpeg_quality: int | None = None
    ):
        ...
        if self._undistort_maps:
            ...
            print(f"[WebcamManager] Initialized WITHOUT undistort maps")
            ...
            if ok and frame is not None:
                ...
                if maps is not None:
                    ...
                    if name not in self._no_undistort_logged:
                        ...
                        self._no_undistort_logged.add(name)
    ...
    def snapshot_latest_views(self) -> dict[str, str]:
        """Snapshot the latest **JPEG base64 strings** for each camera. We copy dict entries to avoid referencing a dict
        being mutated by workers.
